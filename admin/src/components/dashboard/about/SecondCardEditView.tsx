...
import { updateMission, useCreateMission } from 'src/api/backendServies';
...
export default function SecondCardEditView({ initialData }: Props) {
    ...
    if (currentStep === languages.length - 1) {
      try {
        ...
        const response = initialData?.id
          ? await updateMission(initialData?.id || '', finalData)
          : await useCreateMission(finalData);
